...
model = AutoModelForCausalLM.from_pretrained(model_name, trust_remote_code=True,
                                             torch_dtype=torch.float16)  # float16可能导致prob计算下溢全部变成0
...
optimizer = torch.optim.Adam(model.parameters(), lr=1e-5, foreach=False)
scaler = GradScaler()
...
def generate_cot_and_entropy(input_text, max_length):
    ...
    with autocast():
        ...
        for _ in range(max_new_tokens):
            ...
            # outputs.logits(batch_size, seq, vocab_size),这里选seq最后一个token，也就是最新生成的token
            logits = outputs.logits[:, -1, :]
            logits_list.append(logits)
            probs = F.softmax(logits.float(), dim=-1)  # logits临时改成32bit，避免下溢出为0
...
def policy_gradient_update(token_ids, rewards, entropy_mask, logits):
    optimizer.zero_grad()
    with autocast():
        log_probs = []
        for logit in logits:
