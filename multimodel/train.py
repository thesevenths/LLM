...
class VLMConfig(PretrainedConfig):
    ...
    def __init__(self, llm_model_path='F:\model\Qwen-0.6B',
                 vision_model_path='F:\model\siglip-so400m-patch14-384',
...
class VLM(PreTrainedModel):
    ...
    def forward(self, input_ids, labels, pixel_values, attention_mask=None):
        ...
        # (b, 196, d) --> (b, 49, d*4) ; img token compressed from 196  reduce to 49,token dimension up to d*4
        # transformer attention complexity is O(n^2),less token, less compute cost
        image_embeds = image_embeds.view(b, -1, d * 4)
        image_features = self.linear2(F.silu(self.linear1(image_embeds)))
        # let data type the same,otherwise pytorch will through exception
        text_embeds = text_embeds.to(image_features.dtype)
        # image_features insert into text embedding，replace <|image_pad|>
        inputs_embeds = self.merge_input_ids_with_image_features(image_features, text_embeds, input_ids)
        # generate output by vision-text merged input
        # again genetate output by llm_model；due to freeze existed weight,only to update added layer, so it's ok
        outputs = self.llm_model(inputs_embeds=inputs_embeds, attention_mask=attention_mask)
        logits = outputs[0] # token's vocab distribution
        loss = None
        if labels is not None: # inference or backpropagation
            loss_fct = nn.CrossEntropyLoss(ignore_index=self.tokenizer.pad_token_id)
            loss = loss_fct(
                # logits (batch_size, seq_len, vocab_size) to (batch_size * seq_len, vocab_size)
                logits.view(-1, logits.size(-1)),
                labels.view(-1).to(logits.device) # labels (batch_size, seq_len) to (batch_size * seq_len,)
            )
        return CausalLMOutputWithPast(loss=loss, logits=logits)

    def merge_input_ids_with_image_features(self, image_features, inputs_embeds, input_ids):
        # image_features：(b, 49, llm_hidden_size)
        num_images, num_image_patches, embed_dim = image_features.shape
        # image_features replace <|image_pad|> padding token
        # input_ids：(b, seq_len)，include 49  <|image_pad|> token
        # find <|image_pad|> from input_ids by torch.where,shape (b*49,)。
        batch_indices, image_indices = torch.where(input_ids == self.tokenizer('<|image_pad|>')['input_ids'][0])
        # inputs_embeds(b, seq_len, llm_hidden_size),replace padding token
...
if __name__ == '__main__':
    config = VLMConfig(vision_model_path='F:\model\siglip-base-patch16-224', image_pad_num=49)
    model = VLM(config).cuda()
    print(model)
    print(f'模型参数量为：{sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    images_path = 'F:\data\LLaVA-CC3M-Pretrain-595K\images'
    data_path = 'F:\data\Chinese-LLaVA-Vision-Instructions\LLaVA-CC3M-Pretrain-595K\chat-translated.json'
    tokenizer = AutoTokenizer.from_pretrained(config.llm_model_path)
    processor = AutoProcessor.from_pretrained(config.vision_model_path)
    output_dir = 'F:\model\multimodel'
    ...
    trainer.save_model('F:\model\multimodel')
    trainer.save_state()
