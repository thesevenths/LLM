...
optimizer = optim.SGD(model.parameters(), lr=lr, weight_decay=wd, momentum=0.9)  # SGD
...
for epoch in range(epochs):
    model.train()
    optimizer.zero_grad()
    X_train_noisy = X_train + torch.randn_like(X_train.to(device)) * 0.01  # 噪 to device
    ...
    # MuOn sim (Theorem 8: adapt mom if grad low)
    grad_n = torch.norm(grad_W1).item()
    for param_group in optimizer.param_groups:
        param_group['momentum'] = 0.99 if grad_n < 0.05 else 0.9  # high mom explore low grad
    optimizer.step()
    ...
    if epoch % eval_every == 0 or epoch == epochs - 1:
        ...
        with torch.no_grad():
            ...
            elif train_acc >= 0.99 and test_acc > 0.1 and grad_norm > 0.01:  # 宽test捕升
                stage = "III: Interactive"
            else:
                stage = "Transition"
            logs['stage'].append(stage)

            print(f"Epoch {epoch:6d} | Train: {train_acc*100:5.2f}% Test: {test_acc*100:5.2f}% | Train Loss: {train_loss:.4f} Test Loss: {test_loss:.4f} | Stage: {stage}")
...
plt.tight_layout()
plot_path = os.path.join(script_dir, 'grok_burst_final.png')
...
try:
    ...
    model_path = os.path.join(script_dir, 'grok_burst_final.safetensors')
    save_file(model.state_dict(), model_path)
    print(f"Model saved: {model_path}")
except ImportError:
    model_path = os.path.join(script_dir, 'grok_burst_final.pth')
    torch.save(model.state_dict(), model_path)
    print(f"Safetensors no; saved: {model_path}")

print(f"\nFinal Train: {logs['train_acc'][-1]:.4f}, Test: {logs['test_acc'][-1]:.4f}")
print("Stages sample: ", logs['stage'][:10] + ['...'] + [logs['stage'][-1]])